...
starwars %>%
  ...
  group_by(species) %>%
  summarize(n=n, avg_height = mean(height, na.rm = TRUE), avg_mass = mean(mass, na.rm = TRUE), avg_bmi = mean(mass/(height/100)^2)) %>%
  distinct() %>%
  arrange(desc(n))
...
x %>%
  group_by(title) %>%
  summarize(numberOfObjects = n()) %>%
  arrange(desc(numberOfObjects)) %>%
  slice_max(numberOfObjects, n = 3)
```
