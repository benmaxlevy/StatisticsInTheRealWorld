...
Looking at the specific coefficients, we can see that in the parallel slopes model, each `Limit_cat` do offset the y-intercept by values != 0, as indicated by the confidence intervals. On the other hand, in the interaction model, we can see that the `Limit_cat` levels do offset the y-intercept by values != 0, but the slope is also offset by values != 0 except in one case (based on CIs), in addition to the `Income` coefficient being positive instead of negative.
